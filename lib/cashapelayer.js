...
function convertAll(contents, options = {}) {
  ...
  function joinContents(items) {
    let results = '';
    items.forEach(item => {
      results += `let ${varName(item.filename)} = ${item.converted}\n\n`;
    });
    return results;
  }

  return Promise.all(contents.map(content => convert(content.svgContent, options)
      .then(converted => ({ converted, filename: content.filename })
    )))
    .then(separated => ({ output: joinContents(separated), extension }))
}
