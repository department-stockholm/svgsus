...
function convert (inputXml, options = {}) {
  ...
  function recursiveTreeWalkItem (key, current, baseStyle, groupLevel, output) {
    if (key === 'g' && Object.keys.length > 0) {
      ...
      recursiveTreeWalk(
        current,
        appendGroupStyles(current.$, baseStyle),
        groupLevel,
        output
      )
      if (groupIsValid) groupLevel--
      ...
      if (pathD != null) {
        ...
        console.warn(
          'found path(s) without data (empty or invalid parameter <i>d</i>)'
        )
      }
    } else if (key === 'line') {
      printPath(
        ShapeConverter.convertLine(current.$),
        [current.$, baseStyle],
        groupLevel,
        output
      )
      ...
      printPath(
        ShapeConverter.convertRect(current.$),
        [current.$, baseStyle],
        groupLevel,
        output
      )
      ...
      printPath(
        ShapeConverter.convertCircle(current.$),
        [current.$, baseStyle],
        groupLevel,
        output
      )
      ...
      printPath(
        ShapeConverter.convertEllipse(current.$),
        [current.$, baseStyle],
        groupLevel,
        output
      )
      ...
      printPath(
        ShapeConverter.convertPolygon(current.$, true),
        [current.$, baseStyle],
        groupLevel,
        output
      )
      ...
      printPath(
        ShapeConverter.convertPolygon(current.$, false),
        [current.$, baseStyle],
        groupLevel,
        output
      )
      ...
      console.warn(
        '<i>text</i> element is not supported, export all text into path'
      )
    }
  }
      ...
      while ((result = regex.exec(transform))) {
        ...
        if (transformName === 'translate') {
          ...
          console.error(
            "group transform '<i>" +
              transformName +
              "</i>' is not supported, use option <i>Bake transforms into path</i>"
          )
        }
      }
    }
  ...
  function parsePathD (pathData) {
    ...
    if (path.match(/-?\d*\.?\d+e[+-]?\d+/g)) {
      console.warn(
        'found some numbers with scientific E notation in pathData which Android probably does not support. ' +
  ...
  function printGroupStart (groupTransform, groupLevel, output) {
    ...
    output.text += generateAttr(
      'translateY',
      groupTransform.transformY,
      groupLevel + 1,
      0
    )
    output.text += generateAttr(
      'scaleX',
      groupTransform.scaleX,
      groupLevel + 1,
      1
    )
    output.text += generateAttr(
      'scaleY',
      groupTransform.scaleY,
      groupLevel + 1,
      1
    )
    output.text += '>\n'
  ...
  function printPath (pathData, stylesArray, groupLevel, output) {
    ...
    if (styles.hasOwnProperty('transform')) {
      console.warn(
        'transforms on path are not supported, use option <i>Bake transforms into path</i>'
      )
    }
    ...
    output.text += generateAttr(
      'fillColor',
      styles['fill'] && svgColor(styles['fill']).hex(),
      groupLevel,
      'none'
    )
    output.text += generateAttr(
    ...
    output.text += generateAttr(
      'strokeColor',
      styles['stroke'] && svgColor(styles['stroke']).hex(),
      groupLevel,
      'none'
    )
    output.text += generateAttr(
    ...
    output.text += generateAttr(
      'strokeWidth',
      removeNonNumeric(styles['stroke-width']),
      groupLevel,
      '0'
    )
    output.text += generateAttr(
  ...
  function generateAttr (name, val, groupLevel, def, end) {
    ...
    var result = '\n' +
      codeIndent.repeat(groupLevel + 2) +
      'android:' +
      name +
      '="' +
      val +
...
function convertAll (contents, options = {}) {
  ...
  return Promise.all(
    contents.map(content => convert(content.svg, options).then(code => ({
