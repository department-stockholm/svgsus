...
class Swift3Formatter {
  ...
  curve({point, cPoint1, cPoint2}) {
    const fName = this.framework === 'UIKit' ? 'addCurve' : 'curve'
    return `${fName}(to: ${point}, controlPoint1: ${cPoint1}, controlPoint2: ${cPoint2})`
  }
  line({point}) {
    const fName = this.framework === 'UIKit' ? 'addLine' : 'line'
    return `${fName}(to: ${point})`
  }
  closePath() {
    return 'closePath()'
  }
...
function swiftFormatter({version, framework}) {
  if(version == 3) {
    return new Swift3Formatter({framework})
  }
  return new Swift2Formatter({framework})
}

exports.Swift2Formatter = Swift2Formatter
exports.Swift3Formatter = Swift3Formatter
