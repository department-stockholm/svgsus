...
var ShapeConverter = new function () {
  ...
  this.convertRect = function (rectTag) {
    var x = parseFloat(rectTag.x || 0)
    var y = parseFloat(rectTag.y || 0)
    var w = parseFloat(rectTag.width || 0)
    var h = parseFloat(rectTag.height || 0)
    var rx = parseFloat(rectTag.rx || 0)
    var ry = parseFloat(rectTag.ry || 0)
    var r = s(x + w)
    var b = s(y + h)
  ...
  this.convertCircle = function (circleTag) {
    var cx = parseFloat(circleTag.cx || 0)
    var cy = parseFloat(circleTag.cy || 0)
    var r = parseFloat(circleTag.r || 0)
  ...
  this.convertEllipse = function (ellipseTag) {
    var cx = parseFloat(ellipseTag.cx || 0)
    var cy = parseFloat(ellipseTag.cy || 0)
    var rx = parseFloat(ellipseTag.rx || 0)
    var ry = parseFloat(ellipseTag.ry || 0)
  ...
  this.convertPolygon = function (polylineTag, isPolyline) {
    const points = polylineTag.points
    const pointsArrayRaw = typeof points !== 'undefined' ? points.split(',') : []
    const pointsArray = []
    for (let i = 0; i < pointsArrayRaw.length; i++) {
      let splitted = pointsArrayRaw[i].split(' ')
