...
class Swift3Formatter {
  ...
  curve({point, cPoint1, cPoint2}) {
    return `curve(to: ${point}, controlPoint1: ${cPoint1}, controlPoint2: ${cPoint2})`
  }
...
function uiBezierPathParser({pathData, pathName, codeIndent, layer, codeType, skipStart, skipEnd, version}) {
  var points = svgParse(pathData)
  var name = pathName || 'svgsusPath'
  const formatter = version == 3 ?
    new Swift3Formatter({framework: codeType}) :
    new Swift2Formatter({framework: codeType})
